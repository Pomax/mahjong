...
var AI = require('../../apps/client/lib/ai/client');
...
GameManager.prototype = {
  ...
  joinPlayerToGame(socket, data) {
    ...
    socket.emit("joinedgame", { gameid, playerid, ruleset: game.ruleset.name });
    this.notifyGameListUpdate();
  ...
  joinBotToGame(data) {
    // Create a bot, bootstrapped for a specific gameid.
    // It will then connect as any other player via a socket.
    var gameid = data.gameid;
    var game = this.getGame(gameid);
    return new AI(AI.nextId(), this.host, gameid, game.ruleset.getAI());
  },
  ...
  makeNewGame(socket, data) {
    ...
    var game = this.getGame(gameid, data.name, ruleset);
    game.setTimeout(data.timeout);
    socket.emit("madegame", { gameid, ruleset });
  },
