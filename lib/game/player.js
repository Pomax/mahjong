...
Player.prototype = {
   ...
   */
  getDigest() {
    var tcheck = this.tiles.length + 3 * this.revealed.length;
...
//    this.log("verification list", todigest, " - md5:",digest);
    return digest;
  },
  ...
  verify(digest, tiles, bonus, revealed) {
    ...
    if (!passed) {
      this.log("VEFICIATION FAILURE");
      this.log("client", tiles, bonus, revealed);
      this.log("server", this.tiles, this.bonus, this.revealed);
    }
  },
...
//    this.log(playerposition,"was dealt",tileCount,"tiles");
  ...
  deal(tile) {
    ...
    this.tiles = this.tiles.map(v => parseInt(v)).sort();
    this.send("tile", { tile });
    this.log("tiles: ", this.tiles);
  },
