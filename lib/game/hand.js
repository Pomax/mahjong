...
var Hand = function(game, ruleset, handid, players, timeout) {
  // hand data
  this.id = handid;
  this.game = game;
  ...
  if (timeout) { this.discardTimeout = timeout; }
...
//    this.log("player",cpos,"tiles:",this.players[cpos].tiles.sort());
  ...
  handleDiscard(playerposition, tile) {
    ...
    if (playerposition !== this.currentPlayer) {
      console.error("discard mismatch! It's",this.currentPlayer,"'s turn, but we saw a discard by ",playerposition);
      process.exit(1);
    }
    ...
    var timeout = this.discardTimeout || Constants.DISCARD_TIMEOUT;
   ...
   */
  next(discardTile) {
//    this.log("next called. discardTile:",discardTile,"claims:",this.claims.length);
  ...
  processClaims(discardTile) {
    ...
    if (claims.length === 0) {
//      this.log("no valid claims, continuing normal play");
    ...
    this.log("best claim by",best.player.playerposition,"- Awarding",best.claimType,best.winType);
...
//      this.log("notifying players of win");
    ...
    // if it wasn't, the claim is let through and play continues
      ...
      this.log("player",player.playerposition,"claims",tile,"as",claimType);
...
//    this.log("sending player",playerposition,"bonus compensation tile(s)");
    ...
    this.log("player",playerposition,"compensation tile(s):",compensationTiles);
...
//      this.log("notifying players of win");
      players.forEach((player, idx) => {
        player.winOccurred(playerposition, Constants.SELF_DRAWN_WIN, this.players);
      });
      this.log("end of hand.");
