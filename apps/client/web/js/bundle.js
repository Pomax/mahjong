...
/***/ function(module, exports, __webpack_require__) {
	...
	ReactDOM.render(React.createElement(Client, null), document.getElementById('app'));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Wall = __webpack_require__(172);
	var Discards = __webpack_require__(175);
	var Player = __webpack_require__(176);
	var OtherPlayer = __webpack_require__(185);

	var Settings = __webpack_require__(186);

	// externally loaded:
	var io = __webpack_require__(187);
	...
	var Client = React.createClass({
	  ...
	  componentWillMount: function componentWillMount() {
	    ...
	    this.state.socket.on('confirm', function (data) {
	    ...
	    this.state.socket.on("drew", function (data) {
	      _this.setState({ currentPlayer: _this.state.playerposition });
	    });

	    this.state.socket.on("tile", function (data) {
	      var playerposition = data.playerposition;
	      _this.setState({ currentPlayer: playerposition });
	    });
	  },
	  ...
	  formPlayers: function formPlayers(socket) {
	    var _this2 = this;

	    // generate generic player except for ourselves
	    var players = [0, 1, 2, 3].map(function (pos) {
	      if (pos === _this2.state.playerposition) return null;
	      var active = pos === _this2.state.currentPlayer;
	      return React.createElement(OtherPlayer, { ref: "player" + pos, active: active, playerposition: pos, name: _this2.state.playerNames[pos], wind: Player.windKanji[pos], socket: socket, key: pos });
	    });
	    // and then insert "us" into that set based on our play position
	    players.splice(this.state.playerposition, 1, React.createElement(Player, { key: 'player', settings: this.state.settings, socket: socket, playerid: this.state.playerid, gameid: this.state.gameid, ruleset: this.state.ruleset, onNextHand: this.nextHand }));
	    return players;
	  },
	  ...
	  readyGame: function readyGame(data, ruleset) {
	    var _this3 = this;

	    this.setState({ viewLobby: false, playerNames: data.players, ruleset: ruleset }, function () {
	...
	var Lobby = React.createClass({
	  ...
	  componentDidMount: function componentDidMount() {
	    ...
	    this.props.socket.on("madegame", function (data) {
	      _this.joinGame(data.gameid, data.ruleset);
	    ...
	    this.props.socket.on("readygame", function (data) {
	      ...
	      _this.props.readyGame(data, _this.state.ruleset);
	    });
	  },
	  render: function render() {
	    var _this2 = this;
	      ...
	      React.createElement(
	        ...
	        keys.length === 0 ? React.createElement(
	          ...
	          return React.createElement(
	            ...
	            game.players[0] === _this2.state.playername ? React.createElement(
	              'button',
	              { className: 'bot', onClick: function onClick(evt) {
	                  return _this2.addBot(gameid);
	                } },
	              'add bot'
	            ) : null,
	            game.players.length < 4 ? joined ? React.createElement(
	              'button',
	              { className: 'leave', onClick: _this2.leaveGame },
	              'LEAVE'
	            ) : React.createElement(
	              'button',
	              { className: 'join', onClick: function onClick(evt) {
	  ...
	  joinGame: function joinGame(gameid, ruleset) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var scoreplayers = __webpack_require__(171);
	...
	Ruleset.prototype = {
	  ...
	  checkCoverage: function checkCoverage(tiles, bonus, revealed) {
	    ...
	    if (sets < 0) {
	      //       this.log("more than four sets found in", revealed);
	      return false;
	    }

	    if (pair < 0) {
	      //      this.log("multiple pairs found in", revealed);
	      return false;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  ...
	  getSuitTiles: function getSuitTiles(suit) {
	    var set = [];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AI = function AI(client) {
	  ...
	  Object.defineProperty(this, "tiles", {
	    get: function get() {
	      return client.state.tiles.slice();
	    }
	  });

	  Object.defineProperty(this, "onNextHand", {
	    get: function get() {
	      return client.props ? client.props.onNextHand : false;
	    }
	  });
	};
	...
	AI.prototype = {
	  ...
	  updateStrategy: function updateStrategy(discardTile) {
	    var _this = this;
	    ...
	    var text = [];
	    Object.keys(this.required).forEach(function (k) {
	      text.push(k + " (" + _this.required[k].claimType + ")");
	    });
	    //    this.log("required:\n", "  ", text.join(', '));
	  },
	  ...
	  determineClaim: function determineClaim(tile, playerposition) {
	  ...
	  // What do we discard?
	  determineDiscard: function determineDiscard() {
	   ...
	   */
	  updateStrategyNaively: function updateStrategyNaively() {
	    var _this2 = this;
	    ...
	    var required = {};
	    counts.forEach(function (count, tile) {
	      if (count > 1) {
	        var probability = _this2.tracker.getProbability(tile);
	        if (probability > 0) {
	          required[tile] = {
	            claimType: _this2.getClaimTypeFor(tile, count),
	   ...
	   */
	  getClaimTypeFor: function getClaimTypeFor(tile, count) {
	   ...
	   */
	  determineDiscardNaively: function determineDiscardNaively() {
	    var _this3 = this;
	    ...
	    var discards = this.tiles.slice().filter(function (tile) {
	      return !_this3.required[tile];
	    });
	    var discard;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var GameTracker = function GameTracker() {
	  this.tiles = [];
	...
	GameTracker.prototype = {
	  ...
	  playerStart: function playerStart(position, tiles) {
	    //...
	  },


	  // this player gained a known tile
	  gained: function gained(position, tile) {
	  ...
	  discarded: function discarded(position, tile) {
	    //...
	  },


	  // this discard is final, no one is getting this tile.
	  fullyDiscarded: function fullyDiscarded(tile) {
	    // we know the number of available tiles has gone down
	    this.tiles[tile]--;
	  },
	  ...
	  receivedBonus: function receivedBonus(position, tiles) {
	    //...
	  },


	  // another player, not us, claimed a known tile.
	  claimed: function claimed(position, tile, claimType) {
	    //...
	  },


	  // another player, not us, has revealed the following tiles as being out of the game now
	  revealed: function revealed(position, _revealed, concealed) {
	    var _this = this;

	    _revealed.forEach(function (tile) {
	      return _this.tiles[tile]--;
	    });
	  },


	  // what is the probability of getting the following tile, given our tile knowledge
	  getProbability: function getProbability(tile) {
	    var curTotal = this.tiles.reduce(function (a, b) {
	      return a + b;
	    }, 0);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var socketbindings = __webpack_require__(181);

	var Tile = __webpack_require__(173);
	var ClaimMenu = __webpack_require__(182);
	var Overlay = __webpack_require__(183);

	// the static properties we want the Player class to have.
	var statics = __webpack_require__(184);
	...
	var Player = React.createClass({
	  ...
	  resetState: function resetState(done) {
	  ...
	  joinedGame: function joinedGame(gameid) {
	    this.setState({ gameid: gameid });
	  },
	  makeReady: function makeReady(gameid, handid, playerid, playerposition, score) {
	    var _this = this;

	    var ruleset = new rulesets[this.props.ruleset]();
	    var AI = ruleset.getAI();
	    var ai = new AI(this);
	    var state = { gameid: gameid, handid: handid, playerid: playerid, playerposition: playerposition, score: score, balance: '', ai: ai };

	    this.setState(state, function () {
	      _this.send("confirmed", state);
	    });
	  },
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        ...
	        React.createElement(
	          ...
	          React.createElement(
	            ...
	            React.createElement(
	              'span',
	              { className: 'kongs' },
	              this.state.mode === Player.OWN_TURN ? this.state.kongs.map(function (k) {
	                return React.createElement(
	                  'button',
	        ...
	        React.createElement(
	          'div',
	          { className: 'row tiles' },
	          this.renderTiles(this.state.tiles, this.state.mode === Player.HAND_OVER, this.state.dealtTile)
	        ),
	        React.createElement(
	          'div',
	          { className: 'row open' },
	          React.createElement(
	            'span',
	            { className: 'bonus' },
	            this.renderTiles(this.state.bonus, true)
	          ),
	          React.createElement(
	            'span',
	            { className: 'revealed' },
	  ...
	  formOverlay: function formOverlay(winner, loser, draw) {
	    if (this.state.mode !== Player.HAND_OVER || this.state.useAI) {
	      return null;
	    }
	    var content = '';
	    if (draw) {
	      content = "The hand was a draw...";
	    } else if (winner) {
	      content = "You won the hand!";
	    } else if (loser) {
	      content = "Player " + this.state.winner + " won the hand.";
	    }
	    return React.createElement(
	      Overlay,
	      null,
	   ...
	   */
	  renderDiscard: function renderDiscard() {
	    if (this.state.discard === false) {
	      if (this.state.winner !== false) {
	        return React.createElement(
	          'button',
	          { onClick: this.restartReady },
	          'Ready'
	        );
	      }
	      return null;
	    ...
	    if (this.state.claimMenu) {
	      ...
	      return React.createElement(ClaimMenu, { claim: this.claimDiscard, mayChow: mayChow });
	    }
	    ...
	    return React.createElement(Tile, { value: this.state.discard, ownDiscard: ownDiscard, onClick: onClick, title: title });
	  },
	   ...
	   */
	  renderRevealed: function renderRevealed() {
	    var tiles = [];
	    this.state.revealed.forEach(function (set, p1) {
	      set.forEach(function (tile, p2) {
	        tiles.push(React.createElement(Tile, { key: tile + '-' + p1 + '-' + p2, value: tile, title: "revealed tile " + Tiles.getTileName(tile) }));
	      });
	    });
	    return tiles;
	  },
	   ...
	   */
	  renderTiles: function renderTiles(tiles, inactive, tileHighlight) {
	    var _this3 = this;
	    ...
	    return tiles.map(function (tile, pos) {
	      var key = tile + '-' + pos;
	      var onclick = inactive ? null : _this3.handleTileSelect(tile);

	      var highlight = false;
	      if (tile === tileHighlight && _this3.state.mode === Player.OWN_TURN) {
	        highlight = true;
	        tileHighlight = -1;
	      }
	      var ourTurn = _this3.state.mode === Player.OWN_TURN;
	      var title = Tiles.getTileName(tile) + " (" + tile + ")" + (ourTurn && !inactive ? ", click to discard" : '');
	      return React.createElement(Tile, { highlight: highlight, key: key, value: tile, onClick: onclick, title: title });
	    });
	  },
	  ...
	  restartReady: function restartReady() {
	    var _this4 = this;

	    this.resetState(function () {
	      _this4.send("restartready", { ready: true });
	      _this4.props.onNextHand();
	    });
	  },
	   ...
	   */
	  setInitialTiles: function setInitialTiles(tiles) {
	   ...
	   * Add a tile to this player's bank of tiles
	   */
	  addCompensationTiles: function addCompensationTiles(compensation) {
	    var tiles = this.state.tiles;
	  ...
	  otherPlayerDrew: function otherPlayerDrew() {
	    this.setState({
	      discard: false,
	      mode: Player.OUT_OF_TURN
	    });
	  },
	  otherPlayerDiscarded: function otherPlayerDiscarded(discard, discardPlayer) {
	    this.setState({ discard: discard, discardPlayer: discardPlayer });
	  ...
	  otherPlayerClaimed: function otherPlayerClaimed(playerposition, tile, claimType) {
	    this.setState({
	      discard: false
	    });
	  },
	   ...
	   */
	  filterForBonus: function filterForBonus() {
	    var _this5 = this;
	    ...
	    this.setState({
	      ...
	      _this5.log("requesting compensation for", bonus.join(','));
	      _this5.send("compensate", { tiles: bonus });
	    });
	  },


	  // check if this player has any concealed kongs in their hand
	  checkKong: function checkKong(tiles) {
	    var _this6 = this;

	    var counter = {};
	    tiles.forEach(function (t) {
	      return counter[t] = (counter[t] || 0) + 1;
	    });
	    var kongs = Object.keys(counter).filter(function (c) {
	      return counter[c] === 4;
	    });
	    this.setState({ kongs: kongs }, function () {
	      if (_this6.state.useAI) {
	        _this6.determinePlay();
	      }
	    });
	  },
	  ...
	  claimConcealedKong: function claimConcealedKong(tile) {
	    var _this7 = this;

	    return function (evt) {
	      _this7.log("requesting concealed kong for " + tile);
	      _this7.send("claim:concealedkong", { tile: tile });
	    };
	  },
	  allowKongDeclaration: function allowKongDeclaration(tile, compensation) {
	    var _this8 = this;
	    ...
	    if (compensation) {
	      ...
	      this.setState({ tiles: tiles, revealed: revealed }, function () {
	        _this8.addTile(compensation);
	      });
	   ...
	   */
	  handleTileSelect: function handleTileSelect(tile) {
	    var _this9 = this;

	    return function (evt) {
	      if (_this9.state.mode === Player.OWN_TURN) {
	        // players can discard any tile from their playable tile
	        // set during their own turn, but not at any other time.
	        _this9.discardTile(tile);
	      }
	   ...
	   */
	  discardTile: function discardTile(tile) {
	    var _this10 = this;
	   ...
	   */
	  claimDiscard: function claimDiscard(claimType, winType) {
	    var _this11 = this;

	    this.setState({ claimMenu: false }, function () {
	      if (claimType !== Constants.NOTILE) {
	        _this11.send("claim", {
	          tile: _this11.state.discard,
	          claimType: claimType,
	          winType: winType
	        });
	      }
	   ...
	   * Determine which tiles to form a set with.
	   */
	  processClaim: function processClaim(tile, claimType, winType) {
	    var _this12 = this;
	    ...
	    if (claimType <= Constants.CHOW3) {
	      set = this.formChow(tile, claimType);
	    }
	    if (claimType === Constants.PUNG) {
	      set = this.formSet(tile, 3);
	    }
	    ...
	    this.setState({
	      ...
	      _this12.verify();
	      if (_this12.state.useAI) {
	        _this12.determinePlay();
	      }
	    });
	  ...
	  getDigest: function getDigest() {
	    ...
	    this.state.revealed.forEach(function (set) {
	      list = list.concat(set);
	    });
	    var todigest = list.sort(function (a, b) {
	   ...
	   */
	  verification: function verification(result) {
	    if (result === false) {
	   ...
	   */
	  finishDraw: function finishDraw() {
	   ...
	   */
	  finishWin: function finishWin(playerposition, tile, winType) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function(){
	  var crypt = __webpack_require__(178),
	      utf8 = __webpack_require__(179).utf8,
	      isBuffer = __webpack_require__(180),
	      bin = __webpack_require__(179).bin,
...
/***/ function(module, exports) {
	...
	module.exports = {
	  bind: function bind(socket, player, onSocketBound) {
	    ...
	    socket.on('sethand', function (data) {
	      ...
	      player.setInitialTiles(tiles, function () {
	        player.verify();
	      });
	    });
	     ...
	     * or forming a kong)
	     */
	    socket.on('compensation', function (data) {
	      var tiles = data.tiles;
	     ...
	     */
	    socket.on('compensated', function (data) {
	      var playerposition = data.playerposition;
	      var tiles = data.tiles;
	     ...
	     */
	    socket.on('claimed', function (data) {
	      var pos = data.playerposition;
	      var tile = data.tile;
	     ...
	     */
	    socket.on('revealed', function (data) {
	     ...
	     */
	    socket.on('finish:draw', function (data) {
	     ...
	     */
	    socket.on('finish:win', function (data) {
	      var playerposition = parseInt(data.playerposition);
	     ...
	     */
	    socket.on('claim:concealedkong', function (data) {
	      var tile = parseInt(data.tile);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ClaimMenu = React.createClass({
	  ...
	  win: function win() {
	    this.setState({ winclaim: true });
	  },

	  renderWinMenu: function renderWinMenu() {
	    return React.createElement(
	      'div',
	      { className: 'winmenu' },
	      React.createElement(
...
/***/ function(module, exports, __webpack_require__) {
	...
	var OtherPlayer = React.createClass({
	  ...
	  formTiles: function formTiles(tiles, sets) {
	    return tiles.map(function (t, p) {
	      if (!sets) {
	        ...
	        return React.createElement(Tile, { key: t + '-' + p, value: t, title: title });
	      }
	      return React.createElement(
	        'div',
	        { className: 'set' },
	        t.map(function (v, p) {
	          ...
	          return React.createElement(Tile, { key: v + '-' + p, value: v, title: Tiles.getTileName(v) });
	        })
	      );
	    });
	  },
	  ...
	  ours: function ours(data) {
	    return data.playerposition === this.props.playerposition;
	  },
	  addTiles: function addTiles(data) {
	  ...
	  otherPlayerTile: function otherPlayerTile(data) {
	    if (!this.isMounted()) return;

	    this.setState({ ourTurn: false });
	  },
	  addTile: function addTile(data) {
	    if (!this.isMounted()) return;
	  ...
	  removeTile: function removeTile(data) {
	    if (!this.isMounted()) return;
